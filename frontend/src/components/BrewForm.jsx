...
const BrewForm = ({ initialData = {}, onSave, onDelete }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-5 bg-black rounded-xl p-4 sm:p-8">
      ...
      {gramsError && (
        <div className="text-red-400 text-sm font-medium mb-2">{gramsError}</div>
      )}
        <button
          type="submit"
          disabled={!isValid}
          className={`rounded bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 font-semibold transition w-full sm:w-auto ${isValid ? '' : 'opacity-50 cursor-not-allowed'}`}
        >
          Save
        </button>
      </div>
